        ...
        function sendState(screenNumber) {
            ...
            var stuff = connection.query('select * from transactions where transactions.screen = "'+ screenNumber +'" and transactions.group = "'+session.sessionGroup+'"', function(err, rows){
                ...
                for(var i = 0; i<rows.length; i++) {
                    io.sockets.emit('mousedot', {x:rows[i].xpoint, y:rows[i].ypoint, drag:rows[i].drag, rad:rows[i].radius, colour:rows[i].colour, owner:rows[i].owner, group:session.sessionGroup, screen:rows[i].screen, collaborative: true});
                }
            });
        }
            ...
            if(data.intention=="back") {
                ...
                if(screenNumber <3 || screenNumber == minScreen)
                    ...
                    connection.query('select * from screens where id = "' + (screenNumber-1) + '"', function(errr, result) {
                        io.sockets.emit('switchResponse', {response: true, reason:data.intention, bgimage: result[0].bgimage, collaborative:result[0].collaborative, drawable:result[0].drawable, newScreenNumber: screenNumber-1 });
                ...
                connection.query('select max(ID) as "maxval" from screens', function(err, rows){
                    ...
                    if (rows[0].maxval == screenNumber || screenNumber == maxScreen)
                        ...
                        connection.query('select * from screens where id = "' + ( screenNumber +1 ) + '"', function(errr, result) {
                            ...
                            io.sockets.emit('switchResponse', {response: true, reason: data.intention, bgimage: result[0].bgimage, collaborative:result[0].collaborative, max:rows[0].maxval, drawable:result[0].drawable, newScreenNumber: screenNumber+1 });
                            sendState(screenNumber + 1);
...
http.listen(process.env.PORT || 4000, function(){ console.log('listening on *:4000'); });
>>>>>>> 08ec8414882619b9584d240d92539f11c0af4f52
